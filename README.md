...
| 项目要素 | 当前口径 |
|---|---|
| 当前目标 | 在保持公开安全门禁的前提下，继续补齐五个领域的高价值实践内容 |
| 唯一真源 | 本地知识库与飞书知识库是 canonical 母版；`content/` 只保存筛选、脱敏、复核后的公开派生稿 |
| 启动顺序 | 先读 `AGENTS.md`，再读本页与 `docs/ROADMAP.md`；仅在任务需要时读取其他专题文档 |
| 当前阶段 | 阶段 3：内容加深与运营；生产、Git 自动部署、团队与管理员入口均已上线 |
| 默认可执行 | 本地内容编辑、源码修改、构建、门禁、链接与权限测试 |
| 必须单独确认 | 生产部署、域名、付费服务、凭证变更和其他外部系统写入 |
| 完成标准 | 内容门禁、正式构建、dist 泄漏检查、链接检查通过；涉及权限时再通过 `test:access` |
| 当前下一步 | 按 `docs/ROADMAP.md` 的 B2-B5 顺序补齐内容，并滚动复核事实时效 |

> 本表是启动卡，不替代 `AGENTS.md` 的规则、`docs/ROADMAP.md` 的任务状态或 `content-registry.yaml` 的发布登记。
